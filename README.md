...
while keeping the rest of the signal API public. You can do this by locking the signal by calling `lock()`.
...
Slots are a way to provide hooks for common integrations of an interface. Slots can be created from
any function.
	...
	// Now make it a slot.
	log = signals.slot(log);

	// log still works as expected.
	log('Hello World!');

Since slots are just a simple wrapper you can connect them to signals as usual or use the shorthand approach.

	// As usual.
	someSignal.connect(log);

	// Shorthand
	log(someSignal);

Another benefit of slots is that they can be disconnected from all signals they are connected to.

	log.disconnectAll();

*NOTE: This only applies to slots that have been connected using the shorthand approach.*
...
Using signals and slots this can be written once as part of the video player API making it much easier to hook up.

	controls.volume(videoPlayer.volumeChanged);
	controls.mute(videoPlayer.muteChanged);
	videoPlayer.volume(controls.volumeChanged);
	videoPlayer.mute(controls.muteChanged);
	videoPlayer.play(controls.playAction);

The beauty of this architecture is that other components can just as easily be hooked up in a similar fashion.
Now here's the signal and slot code needed to pull this off.
	...
	// Next is a slot for setting the volume state of the controls.
	// This also acts as the getter/setter for volume.
	controls.volume = signals.slot(function (volume) {
		if (volume === undefined) {
			return controls._volume;
		}

		if (volume !== controls._volume) {
			controls._volume = volume;
			controls.volumeSignal.emit(volume);
		}
	});

	// Next is the slot for the mute state of the controls.
	// This also acts as the getter/setter for mute.
	controls.mute = signals.slot(function (mute) {
		if (mute === undefined) {
			return controls._mute;
		}

		if (mute !== controls._mute) {
			controls._mute = mute;
			controls.muteSignal.emit(mute);
		}
	});
	...
	// The volume and mute getter/setters are wrapped in a slot much the same
	// way as the getter/setters of the controls.
	videoPlayer.volume = signals.slot(function (volume) {
		if (volume === undefined) {
			return videoPlayer._volume;
		}

		if (mute !== videoPlayer._mute) {
			videoPlayer._volume = volume;
			videoPlayer.volumeSignal.emit(volume);
		}
	});

	videoPlayer.mute = signals.slot(function (mute) {
		if (mute === undefined) {
			return videoPlayer._mute;
		}

		if (mute !== videoPlayer._mute) {
			videoPlayer._mute = mute;
			videoPlayer.muteSignal.emit(mute);
		}
	});

	videoPlayer.play = signals.slot(function () {
